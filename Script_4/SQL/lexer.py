...
def tokenize(line):
  ...
  recognized = []
  errors = []
  for mtch in re.finditer(r"(?P<CREATE>CREATE)|(?P<TABLE>TABLE)|(?P<IDEN>[a-z]+)|(?P<INTEGER>INTEGER)|(?P<PRIMARY_KEY>PRIMARY KEY)|(?P<TEXT>TEXT)|(?P<LPAREN>\()|(?P<RPAREN>\))|(?P<COLON>,)|(?P<SEMICOLON>;)|(?P<IGNORE>[ \t]+)|(?P<NEWLINE>\n)|(?P<UNKNOWN>.)",line):
